...
/** Lookups already resolved elsewhere: the prebuilt file, or earlier in this session. */
const seeded = new Map<string, ExternalBookInfo | null>();

/**
 * Seeds results generated by `npm run data:covers`. Visitors then see covers
 * with no API call at all, which keeps the site working when the anonymous
 * Google Books quota for their network is exhausted.
 */
export function seedBookInfo(records: Readonly<Record<string, ExternalBookInfo | null>>): void {
  for (const [key, info] of Object.entries(records)) seeded.set(key, info);
}
