...
- 勾選篩選：出版社、適讀年齡、共讀方式、建議標籤、購入管道、狀態、新舊、書況、藏書位置
- 關鍵字輸入：書名、作者、繪者
- **自訂排序順序**：可疊加多個欄位（例如先照書名、書名相同再照價格），也可直接點表格標題排序
- 卡片／表格檢視、每頁筆數，匯出目前結果為 Excel
- 篩選側欄可收合：收起來後左側只留一條窄欄，書單佔滿版面，下次開啟會記得你的選擇
...
| 區域         | 操作                                                                                                         |
| ------------ | ------------------------------------------------------------------------------------------------------------ |
| 左側篩選欄   | 書名／作者／繪者輸入關鍵字；出版社、適讀年齡、共讀方式、建議標籤、購入管道、狀態、新舊、書況、藏書位置用勾選 |
| 條件邏輯     | 同一組條件之間是「或」，不同組條件之間是「且」                                                               |
| 選項後的數字 | 勾選這個條件後會剩下幾本書（已把其他條件算進去）                                                             |
| 收合篩選欄   | 篩選欄標題右邊的按鈕可把側欄滑動收起，左側只留一條窄欄；點窄欄上的按鈕即可展開，窄欄也會顯示目前幾個條件     |
...
| 分享網址     | 篩選條件、排序順序與每頁筆數會寫進網址列，複製給同樣有帳號的人即可看到同一頁                                 |
| 手機         | 上方「篩選」按鈕會叫出篩選抽屜                                                                               |
