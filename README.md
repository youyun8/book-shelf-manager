...
| 每頁顯示     | 結果列右邊的「每頁」可選 12／24／48／96 或全部，下方會出現分頁按鈕         |
| 顯示設定     | 右上角「顯示設定」可調整主題（跟隨系統／淺色／深色）、字級與版面寬度       |
...
| 詳細視窗顯示「查詢額度已用完」       | Google Books 免金鑰配額依 IP 計算。稍後再試、改用 Wi-Fi，或設定 API 金鑰。                                                                                                 |
...
│  │  ├─ display-settings.ts # 主題、字級、版面寬度
