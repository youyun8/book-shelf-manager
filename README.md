...
- 勾選篩選：狀態、購入管道、新舊、書況、位置、出版社、適讀年齡、共讀方式、建議標籤
- 關鍵字輸入：書名、作者、繪者
- **自訂排序順序**：可疊加多個欄位（例如先照書名、書名相同再照價格），也可直接點表格標題排序
- 卡片／表格檢視、每頁筆數，匯出目前結果為 Excel
- 篩選條件獨立成一頁（`/filters`），從上方列的「篩選」進入，書單頁面因此滿版顯示
...
| ------------ | ---------------------------------------------- | --------------------------------------------------------------- |
| 書名         | `書名`、`書籍名稱`、`名稱`、`Title`            | 建議必填，空白列會被略過；**同一個書名可以出現多列**            |
| 狀態         | `狀態`、`書籍狀態`、`收藏狀態`                 | 勾選篩選，例如 收藏／待售／待共讀／共讀中／絕版待售／絕版待交換 |
| 購入管道     | `購入管道`、`購買管道`、`來源`                 | 勾選篩選、可排序                                                |
| 價格         | `價格`、`購入價格`、`售價`、`定價`             | 可排序；沒填價格的書一律排在最後                                |
| 新舊         | `新舊`、`新舊程度`、`成新`                     | 勾選篩選，例如 近新／9新／8新；排序會照新到舊                   |
| 書況         | `書況`、`書本狀況`、`品相`                     | 勾選篩選，例如 無／點斑／微斑；排序會照好到差                   |
| 位置         | `位置`、`藏書位置`、`存放位置`、`書櫃位置`     | 勾選篩選、可排序                                                |
| 備註         | `備註`、`註記`、`筆記`                         | 顯示於書籍詳細視窗                                              |
| 作者         | `作者`、`文`、`Author`                         | 可用關鍵字搜尋、可排序                                          |
| 繪者         | `繪者`、`插畫`、`繪圖`、`Illustrator`          | 可用關鍵字搜尋、可排序                                          |
| 譯者         | `譯者`、`翻譯`                                 | 顯示於卡片與詳細資料、可排序                                    |
| 出版社       | `出版社`、`出版商`、`Publisher`                | 勾選篩選、可排序                                                |
| 內容簡介     | `內容簡介`、`書籍內容摘要`、`內容摘要`、`簡介` | 卡片顯示三行，詳細資料顯示全文                                  |
| 適讀年齡     | `適讀年齡`、`年齡層`、`建議年齡`               | 勾選篩選與排序都依數字大小（0-3 早於 4-10）                     |
| 共讀方式     | `共讀方式`、`共讀`、`閱讀方式`                 | 勾選篩選，例如 幼兒啟蒙／親子共讀／低中年級                     |
| 建議標籤     | `建議標籤`、`分類標籤`、`標籤`、`分類`         | 勾選篩選，可填多個標籤                                          |
| ISBN（選填） | `ISBN`、`ISBN13`、`條碼`                       | 顯示於詳細資料，匯出時一併帶出                                  |

**表格的欄位順序就是網頁的順序**：表格檢視的欄、書籍詳細視窗、編輯表單、排序選單、
篩選頁面與匯出檔都照這個順序排列。想調整順序，改 `src/types.ts` 的 `FIELD_ORDER` 即可，
`src/lib/fields.test.ts` 會檢查各處沒有各自走鐘。

### 一列就是一本

**每一列代表一本實體書，不是一個書名。** 同一本書買了兩本、一本收藏一本待售，就寫成兩列，
兩列的書名相同、狀態不同。網頁完全照這個方式處理：不會把同名的列合併，兩列各自可以編輯或刪除；
卡片與詳細視窗會標示「同書名共 N 本」，詳細視窗還會列出其他幾本的狀態、管道與價格，
依書名排序時同名的幾本會排在一起。

`狀態`、`新舊`、`書況` 是三個不同的欄位：`狀態` 是這本書要做什麼（收藏、待售），
`新舊` 是這本二手書幾成新，`書況` 是書上的斑點或破損。舊檔案若只有 `書況` 一欄、
裡面填的其實是 `收藏`、`待售` 之類的字，匯入時會自動歸到 `狀態`。

其他自訂欄位（例如 `借給誰`）不會被丟掉，會出現在書籍詳細視窗中。
...
- 若檔案裡有兩欄意思重疊（例如同時有「價格」和「購入價格」），程式會挑**實際有填資料的那一欄**當作篩選依據，另一欄會保留在書籍詳細視窗中。
...
| 區域         | 操作                                                                                                                           |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| 篩選頁面     | 上方列的「篩選」進入；書名／作者／繪者輸入關鍵字，狀態、購入管道、新舊、書況、位置、出版社、適讀年齡、共讀方式、建議標籤用勾選 |
| 同書名多本   | 卡片會標「同書名共 N 本」，點進去可看到其他幾本的狀態、購入管道與價格                                                          |
| 條件邏輯     | 同一組條件之間是「或」，不同組條件之間是「且」                                                                                 |
| 選項後的數字 | 勾選這個條件後會剩下幾本書（已把其他條件算進去）                                                                               |
| 回到書單     | 篩選頁右上角的「查看 N 筆結果」，或點左上角的標題                                                                              |
...
| 排序         | 結果列的「排序」可加入多個欄位，由上往下依序套用；也可直接點表格標題                                                           |
...
| 分享網址     | 篩選條件、排序順序與每頁筆數會寫進網址列，兩個頁面都可直接分享或加書籤                                                         |
| 手機         | 上方列只留圖示，「篩選」同樣進入篩選頁面                                                                                       |
...
| 網頁打得開但一直轉圈、主控台出現 500 | 資料表還沒建立，或還沒套用新的欄位（狀態／新舊／書況／備註／共讀方式）。執行 `npm run db:migrate`。                                                                        |
...
│  └─ 0005_split_status_fields.sql # 狀態／新舊／書況／備註 各自成欄
...
│  │  ├─ filter.ts         # 篩選條件
│  │  ├─ sort.ts           # 多欄位排序（使用者自訂順序）
│  │  ├─ vocabulary.ts     # 狀態／新舊／書況／共讀方式的固定選項與順序
│  │  ├─ facets.ts         # 勾選選項與數量統計
│  │  ├─ badge.ts          # 狀態色塊與價格格式
│  │  ├─ display-settings.ts # 主題、色調、字級、版面寬度
│  │  ├─ pagination.ts     # 每頁筆數與分頁按鈕
│  │  ├─ url-state.ts      # 篩選條件與網址列同步
│  │  ├─ route.ts          # 書單／篩選兩個頁面的網址
│  │  ├─ copies.ts         # 同書名的多本書怎麼數、怎麼找
